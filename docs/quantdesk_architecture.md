# QuantDesk — Cognitive Trading Brain

> **QuantDesk Framework · Ruby + Ollama Cloud**
>
...
3. [Tape Reader](#sensory-cortex)
4. [Reasoning & Intent](#reasoning--intent)
5. [Trade Journal](#trade_journal)
6. [Risk CRO](#risk-cro)
...
QuantDesk maps biological trading desks to institutional trading desk roles. Each desk is a decoupled service communicating via a Pub/Sub event bus — never direct calls.

#### Architecture Diagram

```
Binance WebSocket  ──────►  Sensory  ──( tape tick )──►  NervousSystem
                                                                               │
                          ┌────────────────────────────────────────────────────┘
                          │
                          ├──► :market_regime_changed  ──►  Prefrontal (Ollama llama3:70b)
                          │                                                    │ :intent_generated
                          │                                                    ▼
                          │                                           Risk Manager (CRO)
                          │                                                    │ :approved_order
...
#### 🗃️ Trade Journal — Episodic Memory
- **Role:** RAG · VECTOR STORE
- **Description:** Stores every closed trade as a vector embedding of market context + outcome. On each new signal the  performs cosine similarity recall — enabling "déjà vu" about past liquidation cascades or winning setups.
- **Tech:** `qdrant-ruby` · `nomic-embed-text` · `pgvector`

#### 📡 NervousSystem — Event Bus
- **Role:** PUB/SUB DECOUPLING
- **Description:** Lobes never call each other directly. Everything flows through the NervousSystem pub/sub bus using wisper. This makes each desk independently testable, replaceable, and failure-isolated.
- **Tech:** `wisper` · `concurrent-ruby` · `redis streams`

> **Key Design Principle:** The Risk Manager (System 1, hardcoded rules) always has veto power over the Portfolio Manager (System 2, LLM reasoning). This prevents the LLM from hallucinating a "great trade" during a volatility spike or when daily drawdown limits are already breached.
...
gem "wisper",                "~> 2.0"   # Pub/Sub between trading desks
...
### Portfolio Manager — Reasoning & Intent
...
### app/desks/portfolio_manager.rb
...
class Prefrontal
  ...
  def initialize(nervous_system:, trade_journal:)
    @ns     = nervous_system
    @memory = trade_journal
...
### Trade Journal — Episodic Memory & RAG

Unlike standard agents that forget everything on restart, QuantDesk embeds every trade's full market context as a vector and performs cosine recall — enabling genuine learning from past mistakes.

### app/desks/trade_journal.rb
...
class Trade Journal
  ...
  # Called by Prefrontal before generating a trade plan
...
> **Why this works:** Standard agents have no memory between restarts. QuantDesk recalls semantically similar past trades — e.g., a long absorption signal that led to a stop-hunt 3 weeks ago — and injects that context into the LLM's reasoning before the trade plan is generated.
...
| ❌ What Retail Does | ✅ What QuantDesk Does |
|---|---|
...
### Execution Trader — Execution & Binance Client
...
QuantDesk runs a structured daily routine: pre-market macro analysis, intraday execution, post-market post-mortem, and a weekend "dream state" for self-improvement.

### Daily Flow

1. **08:00 UTC — Pre-Market Macro Scan**
   - Scrape economic calendar (CPI, FOMC, Powell). If high-impact event within 30 mins, CRO halves all position sizes or halts trading entirely. Feed overnight news into Trade Journal as background context.

2. **09:00–20:00 UTC — Intraday Execution**
   - Sensory streams Binance WS. Alpha Wave Loop pulses every 60s. Prefrontal evaluates tape signals, recalls episodic memories, generates Trade Plans. CRO gates all plans before Motor executes.
...
require_relative "app/desks/trade_journal"
require_relative "app/desks/tape_reader"
require_relative "app/desks/portfolio_manager"
require_relative "app/desks/risk_manager"
require_relative "app/desks/execution_trader"
require_relative "app/clients/binance_futures_client"
require "wisper"
require "concurrent"

puts "🧠 Booting QuantDesk Cognitive Architecture v1.0..."
...
trade_journal = Trade Journal.new
sensory     = Sensory.new(nervous_system)
cortex      = Prefrontal.new(nervous_system:, trade_journal:)
risk_manager    = Risk Manager.new(nervous_system:, equity: 10_000)
motor       = Motor.new(nervous_system:, binance:)
...
puts "👁️  Sensory online — streaming BTCUSDT tape"
puts "🧠 QuantDesk is awake."
...
class NightlyPostMortem
  def initialize(trade_journal:, config_path:)
    @memory      = trade_journal
...
Every gem used by QuantDesk, organised by brain desk function.

| Gem | Version | Lobe / Role | Purpose |
|---|---|---|---|
| `ruby_llm` | `~> 1.2` | Portfolio Manager | Unified Ollama Cloud interface. JSON mode, tool calling, streaming, embed support. |
| `langchainrb` | `~> 0.8` | Portfolio Manager | ReAct agent loop, vectorstore abstraction, document loaders for RAG ingestion. |
| `oj` | `~> 3.16` | Portfolio Manager | Fast JSON parsing for LLM structured output — 10–20× faster than stdlib JSON. |
| `wisper` | `~> 2.0` | NervousSystem | Pub/Sub event bus. Decouples desks so each can be independently tested and replaced. |
| `concurrent-ruby` | `~> 1.2` | NervousSystem | Thread-safe data structures, TimerTask for the 60s alpha wave heartbeat loop. |
| `async` | `~> 2.8` | Execution Trader | Non-blocking I/O for async TWAP/iceberg execution and WebSocket connections. |
| `async-websocket` | `~> 0.25` | Sensory | Streams aggTrade, depth20, forceOrder from Binance FAPI WebSocket endpoint. |
| `faraday` | `~> 2.9` | Execution Trader | HTTP client for all Binance REST API calls (orders, prices, funding rates, OI). |
| `faraday-retry` | `~> 2.2` | Execution Trader | Auto-retry with backoff on 429 rate limit responses. Binance limit: 2400 req/min. |
| `qdrant-ruby` | `~> 1.2` | Trade Journal | Vector database client. Stores episodic trade memories as embeddings, runs cosine recall. |
| `redis` | `~> 5.0` | Trade Journal | Short-term working memory: current position state, live P&L, session flags (desk open/closed). |
| `pg` | `~> 1.5` | Trade Journal | PostgreSQL for long-term trade logs. Use pgvector extension to combine relational + RAG. |
| `numo-narray` | `~> 0.9` | Sensory | N-dimensional arrays for order book math, CVD calculations, volume profiling. |
| `talib-ruby` | latest | Sensory | ATR, RSI, MACD, Bollinger Bands — raw signal inputs for the reasoning layer. |
| `descriptive_statistics` | latest | Risk Manager | VaR, standard deviation, correlation for CRO risk calculations. |
| `matrix` | stdlib | Risk Manager | Cross-asset correlation matrix for position sizing penalty calculations. |
| `prometheus-client` | latest | Observability | Metrics: intent accuracy rate, tool success, average slippage, P&L, risk breaches. |
| `sentry-ruby` | latest | Observability | Exception tracking. Every unhandled error in any desk is captured with full context. |
...
*Generated from QuantDesk Cognitive Trading Brain v1.0 — Ruby + Ollama Cloud*
