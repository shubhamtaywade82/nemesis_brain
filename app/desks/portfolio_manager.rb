...
class PortfolioManager
  ...
  def initialize(event_bus:, journal:, binance: nil, ollama: QuantDesk.ollama_client)
    @events = event_bus
    @journal = journal
    @binance = binance
    @ollama = ollama
  ...
  def macro_snapshot_updated(snapshot)
    unless QuantDesk::LLM_ENABLED
      log("PM: LLM disabled, skipping macro bias update")
      return
    end

    prompt = macro_bias_prompt(snapshot)
    log("Macro bias prompt sent to LLM") if QuantDesk::VERBOSE_LOGS
    bias = ask_llm(prompt, MACRO_BIAS_SCHEMA)
    log("Macro bias result: #{Oj.dump(bias)}") if QuantDesk::VERBOSE_LOGS
  ...
  def generate_trade_plan(symbol:, direction:, price:, atr_pct:, context:, similar_trades:)
    ...
    log("Prompting LLM for #{symbol} #{direction.to_s.upcase} plan") if QuantDesk::VERBOSE_LOGS
    plan = ask_llm(prompt, TRADE_PLAN_SCHEMA)
    log("Trade plan result: #{Oj.dump(plan)}") if QuantDesk::VERBOSE_LOGS
    plan
  rescue Ollama::Error => e
  ...
  def ask_llm(prompt, schema)
    @ollama.generate(prompt:, schema:, model: QuantDesk::REASONING_MODEL)
  end
  ...
  def fetch_atr_pct(symbol)
    ...
    log("ATR calculation failed: #{e.message}") if QuantDesk::VERBOSE_LOGS
    ATR_FALLBACK_PCT
  end
